...
A/B 처치 효과(treatment effect) 가정:
  ...
  꾸며내는 대신, 개인화 추천의 전환율 개선 효과에 대한 일반적인 업계 통계(개인화
  추천이 전환율/참여도를 약 10~30% 끌어올린다는 다수의 이커머스 업계 보고, 예:
  McKinsey/Salesforce 등의 퍼스널라이제이션 리포트)에서 가장 보수적인 쪽인 +15%
  상대 개선을 채택했습니다. 이 수치는 "Kaggle 실측"이 아니라 명시적 "가정"입니다.
...
def pick_product(species, group, entry_source, allergy_group, care_need):
    """상품 선택 로직 (신규):
    개인화(match score/비교)가 실제로 노출되는 경로 — A그룹의 '추천' 진입 — 에서만
    (1) 유저 반려동물의 알레르겐을 포함한 상품을 배제하고 (2) 관심 건강기능이 있으면
    해당 기능 상품을 우선 매칭합니다. 그 외(검색/GNB 진입, 또는 B그룹)는 v1과 동일하게
    무작위 상품을 노출합니다 — 개인화 미노출 상태를 그대로 재현하기 위함입니다."""
