...
IA v0.3을 기준으로 만든 클릭 가능한 프로토타입이 `prototype/index.html`에 있습니다.
빌드·설치·서버 없이 브라우저로 파일을 열면 동작하는 단일 HTML 파일입니다.

- 데스크톱에서는 iPhone 프레임 안에, 모바일 브라우저에서는 전체 화면으로 표시됩니다.
- 로그인 화면의 **데모 데이터로 둘러보기**를 누르면 보리·나비가 등록된 상태로 시작합니다.
- 우측 하단 **화면 목록** 버튼으로 45개 화면(예외 케이스 포함)에 바로 이동할 수 있습니다.
- **기획 주석 보기** 토글로 각 화면이 IA의 어느 항목에서 왔는지 확인할 수 있습니다.

| 데모 프로필 | 구성 | 확인할 수 있는 것 |
| --- | --- | --- |
...
**🔰 데이터 만드는 게 처음이라면 [`docs/데이터_만들기_가이드.md`](docs/데이터_만들기_가이드.md)부터 읽으세요** — "이 데이터로 뭘 알고 싶은가"를 정하는 것부터 가설·실험 설계·수집 계획서·AI 요청·판정·결과물까지 순서대로 안내합니다.

스크립트를 직접 돌리거나 고친다면 [`docs/데이터_파이프라인_운영_가이드.md`](docs/데이터_파이프라인_운영_가이드.md)를 보세요 — 재생성 시 주의사항, 이미 났던 버그, 표본 설계·판정 기준이 정리돼 있습니다.
